...
{
    public partial class Form_tipo_terreno : Form
    {
        public Form_tipo_terreno()
        {
            InitializeComponent();
        }
        ...
        private void Form_tipo_terreno_Load(object sender, EventArgs e)
        {
        ...
        public DataTable Contenido()
        {
            ...
            String cont_tipo_terreno = "select* from T_Terreno";
        ...
        private int buscar()
        {
            ...
            con.Open();
            DataTable datos = new DataTable();
            string bus = "SELECT* FROM T_Terreno WHERE TIPO_TERRENO=@tipo_terreno ";
            MySqlCommand cmd = new MySqlCommand(bus, con);
            cmd.Parameters.AddWithValue("@tipo_terreno", t_terreno.Text);
            MySqlDataAdapter dat = new MySqlDataAdapter(cmd);
            dat.Fill(datos);
        ...
        private void Agegar_Click(object sender, EventArgs e)
        {
            ...
            if (blanco == 0)  //si es cero no esta en blanco y si es uno esta en blanco
            {
                ...
                if (repetidos == 0) //verifica si no hay repetidos, si no hay repetidos realiza la inserción 
                {
                        ...
                        con.Open();
                        string insercion = "INSERT INTO  T_Terreno (TIPO_TERRENO)VALUES(@tipo_terreno)";
        ...
        {
            MessageBox.Show("El tipo de terreno ya existen ");
        }
        ...
        private void Modi_Click(object sender, EventArgs e)
        {
            ...
            if (blanco == 0)  //si es cero no esta en blanco y si es uno esta en blanco
            {
                ...
                if (repetidos == 0) //verifica si no hay repetidos, si no hay repetidos realiza la inserción 
                {
                    ...
                    {
                        con.Open();
                        string actualiza = "UPDATE T_TERRENO SET  TIPO_TERRENO=@tipo_terreno WHERE ID=@id";
        ...
        private void DEl_Click(object sender, EventArgs e)
        {
            ...
            if (id_ac.TextLength > 0) //verifica que si ya selecciono un elemento
            {
                con.Open();
                DataTable datos = new DataTable();
                string bus = "SELECT* FROM T_Terreno WHERE ID=@id AND TIPO_TERRENO =@tipo_terreno";
                MySqlCommand cmd2 = new MySqlCommand(bus, con);
                cmd2.Parameters.AddWithValue("@id", id_ac.Text);
                cmd2.Parameters.AddWithValue("@tipo_terreno", t_terreno.Text);
            ...
            if (contador > 0 && elentos_vacio() == 0)
            {
                ...
                {
                    con.Open();
                    string delete = "DELETE FROM T_Terreno WHERE ID=@id";
        ...
        private void data_tipo_terreno_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            t_terreno.Text = data_tipo_terreno.CurrentRow.Cells[1].Value.ToString();
        }
