...
{
    public partial class Form_amortizacion : Form
    {
        public Form_amortizacion()
        {
            InitializeComponent();
        }
        ...
        private void Form_amortizacion_Load(object sender, EventArgs e)
        {
        ...
        public DataTable Contenido()
        {
            ...
            String cont_tipo_terreno = "select* from Amortizacion";
        ...
        private int buscar()
        {
            ...
            con.Open();
            DataTable datos = new DataTable();
            string bus = "SELECT* FROM Amortizacion WHERE AMORTIZACION=@amortizacion ";
            MySqlCommand cmd = new MySqlCommand(bus, con);
            cmd.Parameters.AddWithValue("@amortizacion", f_amortizar.Text);
            MySqlDataAdapter dat = new MySqlDataAdapter(cmd);
            dat.Fill(datos);
        ...
        private void Agegar_Click(object sender, EventArgs e)
        {
            ...
            if (blanco == 0)  //si es cero no esta en blanco y si es uno esta en blanco
            {
                ...
                if (repetidos == 0) //verifica si no hay repetidos, si no hay repetidos realiza la inserción 
                {
                    ...
                    con.Open();
                    string insercion = "INSERT INTO  Amortizacion (AMORTIZACION)VALUES(@amortizacion)";
        ...
        {
            MessageBox.Show("El forma de Amortizacion ya existen ");
        }
        ...
        private void Modi_Click(object sender, EventArgs e)
        {
            ...
            if (blanco == 0)  //si es cero no esta en blanco y si es uno esta en blanco
            {
                ...
                if (repetidos == 0) //verifica si no hay repetidos, si no hay repetidos realiza la inserción 
                {
                    ...
                    {
                        con.Open();
                        string actualiza = "UPDATE Amortizacion SET  AMORTIZACION=@amortizacion WHERE ID=@id";
        ...
        private void DEl_Click(object sender, EventArgs e)
        {
            ...
            if (id_ac.TextLength > 0) //verifica que si ya selecciono un elemento
            {
                con.Open();
                DataTable datos = new DataTable();
                string bus = "SELECT* FROM Amortizacion WHERE ID=@id AND AMORTIZACION =@amortizacion";
                MySqlCommand cmd2 = new MySqlCommand(bus, con);
                cmd2.Parameters.AddWithValue("@id", id_ac.Text);
                cmd2.Parameters.AddWithValue("@Amortizacion", f_amortizar.Text);
            ...
            if (contador > 0 && elentos_vacio() == 0)
            {
                ...
                {
                    con.Open();
                    string delete = "DELETE FROM Amortizacion WHERE ID=@id";
        ...
        private void data_tipo_terreno_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            f_amortizar.Text = data_tipo_terreno.CurrentRow.Cells[1].Value.ToString();
        }
